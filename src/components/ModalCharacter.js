...
const ModalCharacter = (props) => {
  ...
  const getMyEquipment = async (bag, id = false) => {
    if(id){
      try{
      ...
      let item_find_inventory = false
      let items = []
  
      if(bag){
        
        const idsOnly = bag.inventory
        .filter(obj => obj !== null) // Filtrer les éléments non définis
        .map(obj => obj.id); // Extraire les identifiants

        if(idsOnly.length > 0){
          try{
            const data = await getAllItemsWithId(idsOnly)
            bag.inventory.map((key, index) => {
              if(key){
                item_find_inventory = data.find(
                  element => element.id === key.id
                ) ?? false 
               
                if(item_find_inventory){
                  item_find_inventory.count = key.count
                  items.push(item_find_inventory)
                }
              } 
            })
            return items 
  ...
  const changeView = async (nameView) => {

    if(nameView === "equipment"){
      setView("equipment")
      setEquipment(false)
      ...
      setEquipment(object)
    } else if(nameView === "inventory") {

      setView("inventory")
      setBags(false)

      const object2 = await Promise.all(props.character.bags.map((key) => {
        if(key){
          return getMyEquipment(key)
        } 
      }))
...
// };
  ...
  useEffect(() => {
    ...
    //   setEquipment(false)
    // }
  },[props.character, apiKey, bags, equipment])
  ...
  if(props.character){
        ...
        <>
          ...
          >
            ...
            <Modal.Body className="modal_character">
              <div onClick={() => changeView("equipment") /*setView("equipment")*/} className={props.character.profession+" showView"}>
                <img alt="" src={equipment_icon}/>
                <span>Équipement</span>
                <ArrowForwardIosIcon/>
              </div>

              <div onClick={() => changeView("inventory") /* setView("inventory") */} className={props.character.profession+" showView"}>
                <img alt="" src={inventory_icon}/>
                ...
                <table className='table_bags'>
                  <tbody>
                      {bags.map((key, index) => {
                        if(key){
                          return(
                            <tr key={'bags_'+index} className="row_inventory">
                              {key.map((key2, index2) => {
                                if(key2){
                                  return(
    ...
    } else if(view === "equipment" && equipment){
        return (
          <>
            <Modal 
              show={props.show} 
              ...
              <Modal.Body style={{backgroundImage: `url(${equipment_background})`}}  className="modal_character">
                <table className="character_equipment">
                    ...
                    {Array(tr).fill(1).map((el, i) =>  
                      <tr  className="equipment_modal row_inventory" key={i}>
